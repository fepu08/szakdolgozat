...
# Gives back the Top-N recommendation list for <user_id>
...
def get_top_n_prediction_for_user(user_id):
    ...
    if user_ratings is None:
        return make_response(
            jsonify({"message": f"No recommendations for user with id: {user_id}"}), 404
        )
...
def get_top_n():
    return make_response(jsonify(top_n), 200)
